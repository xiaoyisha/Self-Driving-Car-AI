...
class MyPaintWidget(Widget):
    ...
    def on_touch_down(self, touch): # putting some sand when we do a left click
        ...
        with self.canvas:
            ...
            n_points = 0
            length = 0
            sand[int(touch.x),int(touch.y)] = 1 # can be removed
    ...
    def generate(self, obj):
        print("generating...")
        bpoints = [10, 590]
        bpoints.extend([1500, 400])
        bpoints.extend([250, 250])
        bpoints.extend([790, 10])
        ############################
        P0 = [10, 590]
        P1 = [1500, 400]
        P2 = [250, 250]
        ...
        for i in range(1001):
            ...
            sand[int(p[0]) - 10 : int(p[0]) + 10, int(p[1]) - 10 : int(p[1]) + 10] = 1
            pointss.extend(p)
            d = 100
            if i > 1 and i % 10 == 0:
                k = (pointss[2*i+1]-pointss[2*i-1])/(pointss[2*i]-pointss[2*i-2])
                if not k:
                    continue
                k = 1/k
                theta = atan(k)
                thetaw = d*cos(theta)
                thetah = d*sin(theta)
                if(thetaw > 0 and thetah > 0):
                    points.extend([p[0]-thetaw, p[1]-thetah])
                else:
                    points.extend([p[0]+thetaw, p[1]+thetah])
        ...
        with self.canvas:
            ...
            Line(points=pointss, width=2)
            self.bezier = Bezier(points=bpoints)
            # bbpoints = [-90, 490, 1300, 300, 50, 150, 590, -90]
            # Bezier(points=bbpoints, width=100)
        # print(self.bezier.points)
...
class CarApp(App):
    ...
    def build(self): # building the app
        ...
        generatebtn = Button(text='genertate', pos=(3*parent.width, 0))
        clearbtn.bind(on_release=self.clear_canvas)
        savebtn.bind(on_release=self.save)
        loadbtn.bind(on_release=self.load)
        generatebtn.bind(on_release=self.painter.generate)
        parent.add_widget(self.painter)
        parent.add_widget(clearbtn)
        parent.add_widget(savebtn)
        parent.add_widget(loadbtn)
        parent.add_widget(generatebtn)
        return parent
